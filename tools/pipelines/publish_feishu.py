...
def execute(target: str, **kwargs) -> str:
    ...
    if not os.environ.get("FEISHU_APP_ID") or not os.environ.get("FEISHU_APP_SECRET"):
        ...
        return "错误：未配置 FEISHU_APP_ID 和 FEISHU_APP_SECRET"
    ...
    if wiki_space:
        ...
        log.info("publishing to wiki", extra={"space": wiki_space})
    else:
        log.info("publishing to my space")
    ...
    try:
        ...
        if result.returncode != 0:
            log.error("write_feishu failed", extra={"stderr": result.stderr[:200]})
            return f"错误：发布失败 - {result.stderr[:200]}"

        url = result.stdout.strip()
        log.info("publish_feishu success", extra={"url": url})
        return f"✅ 已发布到飞书\n{url}"

    except subprocess.TimeoutExpired:
        log.error("write_feishu timeout")
        return "错误：发布超时"
    except Exception as e:
        log.error("write_feishu exception", extra={"error": str(e)})
        return f"错误：{str(e)}"
